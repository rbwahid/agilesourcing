...
- **API-First Design**: All endpoints under `/api/` with Sanctum authentication
- **Service Layer**: Business logic in `app/Services/` organized by domain (AI, Admin, Auth, etc.)
- **Models**: Eloquent models with Spatie Permission for RBAC
- **Queue Jobs**: Background processing via `app/Jobs/` with Horizon monitoring

Key models: User, Profile, Design, DesignVariation, Validation, Supplier, SupplierCertification, Conversation, Message, Plan

### Frontend Structure

- **App Router**: Next.js 14 with route groups `(auth)`, `(dashboard)`, `(supplier)`, `(admin)`
...
**Frontend**: @tanstack/react-query, zustand, axios, react-hook-form, zod, recharts, sonner (toasts)
